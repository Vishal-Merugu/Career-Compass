...
class BaseWorkflow {
  ...
  // ─── Run History (Synced to Backend) ─────────────────────────

  async _archiveRun() {
    // Only massConnector is running locally now, so we POST to backend
    await apiSync('/api/sync/workflow-run', 'POST', {
      workflowType: this.id,
      status: this.status,
      params: this._params,
      results: this.results,
      errors: this.errors,
  ...
  async getHistory() {
    const data = await apiSync(`/api/sync/workflow-history?type=${this.id}`);
    return data?.history || [];
  }
